...
* Scrying.ai Project Setup (Azure & Terraform) 
** TODO Set up scrying.ai domain with Azure
   - [ ] Create a new Azure AD (Microsoft Entra ID) tenant:
     - [ ] Go to Azure portal → Microsoft Entra ID → Manage tenants → Create.
     - [ ] Choose "Azure AD" as the tenant type.
   - [ ] Verify scrying.ai domain in Entra ID:
     - [ ] In Microsoft Entra ID, select "Custom domain names."
     - [ ] Add custom domain: scrying.ai.
     - [ ] Follow DNS verification steps by adding provided DNS records at your domain registrar.
...
* Terraform Project Setup for Multi-Subscription Architecture 
